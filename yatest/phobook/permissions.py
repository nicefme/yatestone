...
class IsOwnerOrAdminOrReadOnlyEml(BasePermission):
    def has_object_permission(self, request, view, obj):
     ...
     #   ss
        return request.method in SAFE_METHODS or author == request.user or request.user.is_staff
...
class IsOwnerOrAdminOrModeratorOrReadOnlyForEmp(BasePermission):
    def has_object_permission(self, request, view, obj):
       ...
       ## e = request.user.id in moderators_list
       # ss
        return request.method in SAFE_METHODS or author == request.user or request.user.is_staff or request.user.id in moderators_list